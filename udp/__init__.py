...
MAX_CLIENTS = int(os.environ.get("MAX_CLIENTS")) if os.environ.get("MAX_CLIENTS") != None else float("inf")
# auth
...
printHandler.setFormatter(logging.Formatter(f"{bcolors.OKBLUE}%(threadName)s{bcolors.ENDC} - %(message)s"))
logger.addHandler(printHandler)

fileHandler = logging.FileHandler("paperclip.log")
fileHandler.setLevel(logging.DEBUG)
fileHandler.addFilter(ColorFilter())
fileHandler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(threadName)s - %(message)s'))
